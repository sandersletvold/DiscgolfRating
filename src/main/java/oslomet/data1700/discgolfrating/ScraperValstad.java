...
public class ScraperValstad {
    ...
    public void valstad() {
        ...
        int minIndex = 34;
        int maxIndex = 93;
        valstad = new ArrayList<>(Collections.nCopies(maxIndex - minIndex + 1, null));
        for (int i = 0; i < minIndex - 1; i++) {
            valstad.add(0);
        }
        valstad = new ArrayList<>(Collections.nCopies(maxIndex + 1, 0));
                    ...
                    for (int i = 0; i < columns.size(); i += 2) {
                        if (i + 1 < columns.size()) {
                            try {
                                ...
                                int listIndex = index;
                                if (listIndex >= 0 && listIndex < valstad.size()) {
                                    valstad.set(listIndex, value);
                                } else {
