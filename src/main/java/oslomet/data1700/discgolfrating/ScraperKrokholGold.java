...
public class ScraperKrokholGold {
    ...
    public void krokholGold() {
        ...
        int minIndex = 53;
        int maxIndex = 112;
        krokholGold = new ArrayList<>(Collections.nCopies(maxIndex - minIndex + 1, null));
        for (int i = 0; i < minIndex - 1; i++) {
            krokholGold.add(0);
        }
        krokholGold = new ArrayList<>(Collections.nCopies(maxIndex + 1, 0));
                    ...
                    for (int i = 0; i < columns.size(); i += 2) {
                        if (i + 1 < columns.size()) {
                            try {
                                ...
                                int listIndex = index;
                                if (listIndex >= 0 && listIndex < krokholGold.size()) {
                                    krokholGold.set(listIndex, value);
                                } else {
